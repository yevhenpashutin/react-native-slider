...
export class Slider extends PureComponent<SliderProps, SliderState> {
    ...
    _getValue = (gestureState: typeof GestureState) => {
		...
		const spacesBetween = containerSize.width / ((maximumValue - minimumValue) * 2);
		...
		if (step) {
			for (let i = minimumValue; i <= maximumValue; i = i + step) {
				if (thumbLeft < spacesBetween + spacesBetween * 2 * i) {
					return i;
					break;
				} else {
				}
			}
			 return Math.max(minValue, Math.min(maxValue, minimumValue + Math.floor((ratio * (maximumValue - minimumValue)) / step) * step));
		}
